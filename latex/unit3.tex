...
\begin{document}
    ...
    \hypertarget{unit-3-control-flow-and-list-comprehensions}{%
\section{Unit 3: Control flow and list
comprehensions}\label{unit-3-control-flow-and-list-comprehensions}}
...
\textbf{not} included. We can explicitly specify the start value and
...
\PY{c+c1}{\PYZsh{} How many iterations will be performed? No obvious ex ante.}
...
\end{tcolorbox}
    ...
    Alternatively, we can also create a dictionary:
...
Let \(\alpha \in (0,1)\). The infinite sum of the geometric series
...
use a \texttt{while} loop that continues as along as \texttt{lbound} and
