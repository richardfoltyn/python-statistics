...
\begin{document}
    ...
    \hypertarget{unit-6-advanced-numpy}{%
\section{Unit 6: Advanced NumPy}\label{unit-6-advanced-numpy}}
...
data in a way very similar to low-level languages such as \emph{C} and
\emph{Fortran}. This means that
...
\item
  ...
  adjacent array elements are actually guaranteed to be next to each
  other in memory, which allows for much more efficient computations.
\item
  NumPy array support numerous operations used in scientific computing.
...
You can see performance uplift provided by NumPy arrays in this simple
example:
...
\end{tcolorbox}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
201 µs ± 1.51 µs per loop (mean ± std. dev. of 7 runs, 1000 loops each)
    \end{Verbatim}
...
\end{tcolorbox}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
1.07 µs ± 21.2 ns per loop (mean ± std. dev. of 7 runs, 1000000 loops each)
    \end{Verbatim}
...
\item
  ...
  equivalent array-based operation takes only abount 1 microsecond.
  NumPy is therefore approximately 200 times faster!
\item
...
Many of the array creation take the desired shape of the array as their
first argument. Array shapes are usually specified as tuples:
...
The built-in type \texttt{float} has neither a \texttt{shape}, nor an
...
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} 3d\PYZhy{}array}
\PY{n}{x} \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{ones}\PY{p}{(}\PY{p}{(}\PY{l+m+mi}{1}\PY{p}{,} \PY{l+m+mi}{2}\PY{p}{,} \PY{l+m+mi}{3}\PY{p}{)}\PY{p}{)}
\PY{n}{x}
\end{Verbatim}
\end{tcolorbox}
...
axis of length \(n\), the index

\begin{itemize}
\tightlist
\item
  must only contain integers between \(0\) and \(n-1\), or negative
  integers from \(-n\) to \(-1\) (which, as usual, count from the end of
  the axis).
\item
  can be of arbitrary length. We can therefore select the same element
  multiple times.
\end{itemize}
...
\end{tcolorbox}
    ...
    With multi-dimensional data arrays, the same restrictions apply, and
additionally - if more than one axis is indexed using index arrays, the
index arrays have to be of equal length.
...
\end{tcolorbox}
    ...
    \emph{Case 3:} Applying element-wise functions This case covers numerous
functions defined by NumPy, such as - \texttt{np.sqrt}: square root -
\texttt{np.exp}, \texttt{np.log}, \texttt{np.log10}: exponential and
logarithmic functions - \texttt{np.sin}, \texttt{np.cos}, etc.:
...
\paragraph{Array multiplication}\label{array-multiplication}}
...
\href{https://numpy.org/doc/stable/reference/generated/numpy.dot.html}{docs}).
...
allows us to use arrays with different shares that can be ``broadcast''
to identical shapes.

What do we mean by ``broadcast''? - Imagine we want to add a
\(2 \times 3\) matrix to a length-2 vector. - This operation does not
make sense, unless we interpret the (column) vector as a \(2 \times 1\)
matrix, and replicate it 3 times to obtain a \(2 \times 3\) matrix. This
is exactly what NumPy does.
...
\end{tcolorbox}
    ...
    Observe that even though the resulting array is
\texttt{{[}3.0,\ 3.0,\ 3.0{]}} and can thus be represented as integers
without loss of data, NumPy only takes into account that one of the
operands is floating-point, and thus the result has to be of
floating-point type!
...
\item
  ...
  Unlike with floating-point, the integer representation is
  \emph{exact}, but covers a much smaller range (and, obviously, no
  fractional numbers)
\item
...
\item
  ...
  Even if you are not using any dedicated CPUs or GPUs, data has to be
...
in \emph{C}, \emph{Fortran} or \emph{Matlab}):
...
points to the same block of memory. It does this for performance reasons
...
elements in \texttt{x}. For example, we can let \texttt{y} be a view on
every \emph{second} element in \texttt{x}:
...
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} Copies are creating with integer array indexing}
\PY{n}{x} \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{arange}\PY{p}{(}\PY{l+m+mi}{10}\PY{p}{)}
...
\end{tcolorbox}
    ...
    As an alterantive to \texttt{np.copy()} we can directly call the
...
\item
  ...
  order}, because its the same as in \emph{C} programming language)
\item
  ...
  \emph{Fortran} programming language)
\end{enumerate}
